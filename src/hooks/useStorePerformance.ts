...
export function useStorePerformance(days: number = 30) {
  const [data, setData] = useState<StorePerformance[]>([]);
  ...
  async function fetchPerformance() {
    try {
      ...
      logger.info(`Store performance calculated: ${performance.length} stores analyzed`);
      setData(performance);

    } catch (err) {
      logger.error('Failed to fetch store performance', err);
